...
def func_obj(x):
    ...
    #f = quadratic(x)
    f = np.square(2.5*x[:,3] -1.5* x[:,4] + x[:,5]) + np.square(x[:,0]-0.5) + np.exp(-np.square(x[:,1]-0.5)/0.05) + 1/(1+10*x[:,2])
...
if __name__ == '__main__':   
    ...
    if 1:
        ...
        save_name = file_path + name + 'add' + '.png'
        h = 1e-2
        ...
        for i in add_id:
            ...
            if cnt == 0:
                ax.set_ylabel('f')
            cnt += 1
        plt.suptitle('visualization for additive variable')
        plt.savefig(save_name)
        plt.show()

    #Variable Selection
    grid_var = lhs(func_d, samples=100, criterion='m')
    var_index = variable_selection(func.obj, grid_var, N_j = 10, grid_length = 0.01)
    ...
    x_val = np.random.uniform(0, 1, size = [int(1e3), func_d])
    ...
    num_y = func.n
    num_x = func.N1
    grid_mds = func.generate_grid(func_d-1, 50)
    disjoint_group = [[var_index[i]] for i in range(func_d)]
    grid_mds = func.generate_grid(func_d-1, func.n)
    nonadd_id = [i for i in range(dim) if i not in func.add_id]
    for stage in np.arange(1, func_d):
        ...
        h_show = func.stage_plot(stage, x_plot, y_plot, nonadd_id[var_index[stage-1]], var_index, name, exploration, nonadd_id)
