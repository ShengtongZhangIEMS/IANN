...
"""   
def CCS2(sample_num, dim, alpha, index, Nj = 10, grid_length = 0.05, x0 = None):
    '''
    sample_num: the number of samples.
    dim: dimension of the samples.
    alpha: distance between two neighboring grids.
    index: index of j
    x0: the starting samples with shape (n0, dim)
    Nj: number of scatter point in the x_j axis
    grid_length: length of grid for finite difference
    '''
    sample_idx = 0
    id = 0
    x = np.zeros((sample_num, dim))
    if x0 is None:
        n0 = 0
    else:
        n0 = len(x0) #number of starting samples
    while sample_idx < sample_num:
        if id >= n0:
            # randomly generate a starting point
            x_st = np.random.uniform(0, 1, dim)
            x[sample_idx, :] = x_st
            sample_idx += 1
        else:
            x_st = x0[id, :]
        ...
        x_k = np.array(x_st)
        flag_st = True # check whether x_k is a starting point
        e_last = np.zeros((dim)) # direction at the last sample
        while sample_idx < sample_num:
            A = np.zeros((dim, dim))
            for i in range(Nj):
                f0 = obj(np.reshape(np.insert(x_k, index, i/Nj), (1,-1)))
                #print(x更改function 变量中的值_k)
                gradient = grad(x_k, index, i/Nj, grid_length)
                A += np.dot(gradient, gradient.transpose())
            ...
            if flag_st:
                e_st = e
                flag_st = False
            elif np.dot(e_last, e.transpose()) < 0:
                e = -e
            x_new = x_k - alpha * e[:]
            # determine if x_new is inside the domain [0,1]^d.
            flag = [0< i < 1 for i in x_new]
            if np.all(flag):
                x_k = x_new
                x[sample_idx, :] = x_k
                e_last = e
                ...
                flag_st = True
                break
        # Take samples along another direction
        x_k = np.array(x_st)
        while sample_idx < sample_num:
            A = np.zeros((dim, dim))
            for i in range(Nj):
...
def backward_indexPCA(obj, grid, grad_index, nongrad_index, candidate_index, N_j = 50, grid_length = 0.05):
    ...
    dim_cand = len(candidate_index)-1
    ...
    if dim_cand > 0:
        x_grad = lhs(dim_cand, samples=N_j, criterion = 'm')
    ...
    for ind in candidate_index:
        ...
        for i in range(sample_num):
            ...
            err += orth.dot(np.transpose(orth))
        grad_norm += LA.norm(gradient)**2 # Calculate the gradients L^2 norm.
        E1 += [np.mean(err)/grad_norm]

        # Calculate the second projection error
        if dim_cand > 0:
            cand_ind = list.copy(candidate_index)
            cand_ind.remove(ind)
            noncand_ind = [i for i in range(dim) if i not in cand_ind]
            err=np.zeros(N_j)
            grad_norm = 0
            for l in range(N_j):
                gradient = np.zeros((sample_num, dim_cand))
                for i in range(sample_num):
                    gradient[i, :] = grad(obj, x_grad[l,:], noncand_ind, grid[i, noncand_ind], grid_length).T
                cov_mat = np.dot(gradient.T,gradient)
                eigval, eigvec = LA.eig(cov_mat)
                idx = eigval.argsort()[::-1]   
                eigval = np.real(eigval[idx[:k]])
                ...
                for i in range(sample_num):
                    orth = gradient[i, :] - gradient[i, :].dot(np.transpose(eigvec.flatten())) \
                            * eigvec.flatten()
                    err[l] += orth.dot(np.transpose(orth))
                grad_norm += LA.norm(gradient)**2
            grad_norm /= sample_num
            E2 += [np.mean(err)/grad_norm]
        else:
            E2+=[0]
...
def greedy_disjoint_subspace(obj, dim, samples, init_index = [], N_j = 50, grid_length = 0.01, eta = 1e-4):
    """
    ...
    while len(index)>0:
        if cnt > 0:
            ...
            grad_index = init_index
        candidate_index = [element for element in index if element not in grad_index]
        nongrad_index = [element for element in total_index if element not in grad_index]
        while len(candidate_index) > 0:
            grad_index, nongrad_index, E = backward_indexPCA(obj, grid, grad_index, nongrad_index, candidate_index, N_j = N_j, grid_length=grid_length)
            print(np.min(E))
            print(grad_index)
            print(nongrad_index)
            if np.min(E) > eta:
...
if __name__ == '__main__':
    ...
    dg = greedy_disjoint_subspace(obj, dim, 100, init_index = [0], grid_length = 0.01, eta = 0.05)
    print(dg)
    '''
    dg = {}
    for i in range(dim):
        dg[str(i)] = greedy_disjoint_subspace(obj, dim, samples, init_index = [i], N_j = 50, grid_length = 0.01, \
                                                eta = 1e-4)
    print(dg)
    '''
