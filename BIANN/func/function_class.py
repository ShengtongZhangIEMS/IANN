...
class function():
    ...
    def overlap_plot(self, model_1, model_2, model_dc, x_plot, J0, J1, J2, num_x, labels, name, heatmap = False):
        ...
        if len(J0+J1) == 1:
            i = J1[0]
            h1_min, h1_max = self.lb[i], self.ub[i]
        else:
            h1_min, h1_max = np.percentile(h1, 5), np.percentile(h1, 95)
        if len(J0+J2) == 1:
            i = J2[0]
            h2_min, h2_max = self.lb[i], self.ub[i]
        else:
            h2_min, h2_max = np.percentile(h2, 5), np.percentile(h2, 95)
