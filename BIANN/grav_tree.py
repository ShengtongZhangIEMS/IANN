...
def func_obj(x_input):
    ...
    f = (x[:,0] * x[:,1] * x[:,2]) / (np.square(x[:,3]) + np.square(x[:,4]) + np.square(x[:,5]) )
    # f = np.log(f+1)
    return np.clip(f.reshape((-1, 1)), None, 500)
...
if __name__ == '__main__':
    ...
    group_indices = variable_selection_tree(func.obj, grid_var, lb, ub, grid_length = 0.0001)
    ...
    # r2 = 1 - np.var(np.exp(y_pred) - np.exp(y_test)) / np.var(np.exp(y_test))
